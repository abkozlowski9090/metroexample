...
function wpcf7_constant_contact_submit( $contact_form, $result ) {
	...
	$additional_setting = reset(
		$contact_form->additional_setting( 'constant_contact' )
	);

	if ( in_array( $additional_setting, array( 'off', 'false', '0' ), true ) ) {
		$do_submit = false;
	}
...
class WPCF7_ConstantContact extends WPCF7_Service_OAuth2 {
	...
	public function load( $action = '' ) {
		...
		if ( $this->is_active() ) {
			...
			foreach ( (array) $this->get_contact_lists() as $list ) {
				...
				if ( isset( $this->contact_lists[$list_id] ) ) {
					$list['selected'] =
						! empty( $this->contact_lists[$list_id]['selected'] );
				}
		...
		if ( 'edit' == $action and 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			foreach ( $this->contact_lists as $key => $list ) {
				$this->contact_lists[$key]['selected']
					= in_array( $list['list_id'], $list_ids );
			}
			...
			foreach ( $this->contact_lists as $list ) {
				echo sprintf(
					...
					wpcf7_format_atts( array(
						...
						'checked' => empty( $list['selected'] ) ? '' : 'checked',
					) ),
